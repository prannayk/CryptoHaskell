...
doit xs ke 
 | (length ls)<16 = (des (ls ++ (take (16-(length ls)) (repeat '0'))) sl)
 | (length ls)>=16 = (des (take 16 ls) sl) ++ (doit (drop 8 xs) ke) 
  where 
  		ls = hex xs
  		sl = take 16 $ hex ke

doit2:: [Char]->[Char]->[[Char]]
doit2 xs ke 
 | (length ls)<16 = unhex (indes (ls ++ (take (16-(length ls)) (repeat '0'))) sl)
 | (length ls)>=16 = (unhex (indes (take 16 ls) sl)) ++ (doit2 (drop 8 xs) ke) 
  where 
  		ls = hex xs
  		sl = take 16 $ hex ke
--xor operation
